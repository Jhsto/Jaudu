...
head.js("//cdnjs.cloudflare.com/ajax/libs/jquery/1.8.2/jquery.min.js", "//cdnjs.cloudflare.com/ajax/libs/socket.io/0.9.10/socket.io.min.js", "//cdnjs.cloudflare.com/ajax/libs/modernizr/2.6.2/modernizr.min.js", function() {

  	var socket = io.connect('ws://127.0.0.1:3012');
  	var jauduForm = 0;
  	var currentForm;
	...
	$(document).ready(function() {
		...
		$('form').each(function(){
			...
			$(this).append('<div class=' + jauduForm++ + ' id="jaudu"></div>');

				$(this).submit(function(event){

				currentForm = $(this).children('#jaudu');

				if ($(this).children('#jaudu').html().length <= 0) {
		...
		socket.on('abort', function (data) {
			...
			if (data.wait !== 'OK') {
				alert('You are doing that too much, try again in ' + data.wait + ' seconds.');
			}

			else {
				socket.emit('decryption');
				socket.on('keys', function (data) {
				$(currentForm).append(data.secret);
				$('#jaudu_public').val($('#jaudu_public').val() + data.public);
				$('#jaudu_public').change($(currentForm).parent().submit());
				});
			}
